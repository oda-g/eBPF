...
例えば、このコードでは、dataは、skb(これは、bpfプログラムの引数で渡ってくる値)のオフセット76(__sk_buff の data)へのアクセスとして、バイトコードが出力される。カーネルのロード時、この76という値が、本物のsk_buff構造体のdataメンバのオフセット(216)に変更される。(ロード後のバイトコードは、「bpftool prog dump xlated <id>」で確認できるので、mes_pkt.s と比べてみればよい。)
